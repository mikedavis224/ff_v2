        ...
        {MENUITEMS.slice(0, 3).map((menuItem, i) => {
          return (
            <li key={i} className={` ${menuItem.megaMenu ? "mega-menu" : ""}`}>
              {menuItem.sidebartitle ? <div className="dropdown">{menuItem.sidebartitle}</div> : ""}
              {menuItem.type === "sub" ? (
              ...
              {menuItem.type === "link" ? (
                <Link href={`${process.env.PUBLIC_URL}${menuItem.path}`} className={`${menuItem.active ? "active" : ""}`} onClick={() => toggletNavActive(menuItem)}>
                  <span>{menuItem.title}</span>
                              ...
                              <ul className={`menu-icon ${megaMenuItem.title ? "openSubChildMenu" : ""}`}>
                                {menuItem.title === "Elements"
                                  ? megaMenuItem.children.map((subMegaMenuItem, i) => {
                                      return (
                                        <li key={i}>
                                          <a href={subMegaMenuItem.path} className="elements">
